...
# OVERRIDE: InfoPath displays "Yes" but saves "3" or "N/A" for these specific fields
CUSTOM_MAPPINGS = {
...
def clean(v):
    ...
    # Convert actual line breaks to HTML <br> tags for PowerApps Rich Text
    return re.sub(r"[\r\n]+", "<br>", re.sub(r"[ \t]{2,}", " ", v)).strip()
...
def run():
    ...
    for n, path in enumerate(xmls, 1):
        ...
        try:
            ...
            # STEP 4: 1/2/3 -> Yes/No/N/A
            for c in YESNONA_COLUMNS:
                if row.get(c):
                    # Check if this column has a custom override rule first
