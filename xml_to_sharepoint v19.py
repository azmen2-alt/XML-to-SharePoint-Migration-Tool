...
"""
# ---------------------------------------------------------------------------
# IMPORTS - borrowing code someone else wrote, so we don't rewrite it.
#
...
# uploaded. Confirmed blank in all 2,901 rows of the v18 export.
#   group, test  leftovers from an earlier template revision
# FSCertification is ALSO blank in every row but is NOT dropped here - the
# business has not yet confirmed whether it should hold values. Check the
# "XML FIELDS WITH NO COLUMN" section of the report before deciding.
...
#                  must be followed by " - ", because that is the only thing
#                  that tells an application number apart from a corporation
#                  number. Compare these two real folder names:
#                      "13203 - My Money Tree Inc - Renewal - 2022"
#                           13203 IS the application number
#                      "2611314 Ontario Limited-New-2019"
#                           2611314 is the CORPORATION number. There is no
#                           application number in this folder name at all.
#                  v18 used "2 to 7 digits" with no separator check, so it
#                  wrote a corporation number into ApplicationFolderID for 78
#                  folders. Those values looked perfectly valid and were not
#                  detectable downstream. DO NOT relax this pattern.
#
#   FOLDER_TAIL    The other folder convention, "Business-Type-Year", used by
#                  697 folders. Splits off the type and year so the business
...
def long_path(p):
    ...
    p = os.path.abspath(p)
    # Already prefixed? Leave it, or we would prefix it twice.
...
def parse_xml(path):
    ...
    try:
        ...
        # It failed. Read the raw bytes so we can try to clean them up.
        ...
        # byte we cannot decode becomes a placeholder instead of crashing.
...
def flatten(root, column_keys):
    ...
    # time you use a new key, so we never have to check "does this key exist
    # yet?". set() is an unordered bag with no duplicates.
...
def run():
    ...
    for n, path in enumerate(xmls, 1):
        ...
        try:
            ...
            # ApplicationFolderID holds a NUMBER or NOTHING. v18 fell back to
            # "app_folder", putting the folder name into an ID column for 619
            # rows. A name cannot be matched against a number, so a comparison
            # against SharePoint reported those rows as missing records when
            # they were present all along. The folder name is still kept - in
            # ApplicationFolderName, its own column, where it belongs.
    ...
    # saved beside the first. v18 wrote a row for each, so the destination
    # received two records for one application and no error was raised.
    #
    ...
    #
    # The most recently modified file wins. Everything dropped is listed in
    # the report by name and folder, and counted in the balance check, so
    # nothing goes missing quietly. Read that list: two folders holding the
    # same ApplicationID is a filing problem on the drive, not a tool problem,
    # and this only hides it from the upload - it does not fix it.
    ...
    # ---- SCREEN SUMMARY. One output FILE only: the CSV above.
    # The report file was removed by request. Everything that decides whether
    # the export is safe now prints here instead, so nothing fails silently.
    # If you want a copy, redirect the screen output when you run it:
    #     python xml_to_sharepoint_v19.py > run_log.txt
    L = "=" * 62
    def w(*ls):
        for l in ls: print(l)
